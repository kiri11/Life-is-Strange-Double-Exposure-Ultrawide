...
use crate::games::{self, Plan};
use crate::scan::{Image, Section};
use crate::win;
use crate::{VERSION, config, pe};
...
pub unsafe fn attach(hinst: *mut c_void) {
    ...
    let Some(game) = games::game_for(&exe_name) else { return };
...
unsafe fn run(game: &games::Game, dir: &Path, log: &mut String) -> Result<usize, String> {
