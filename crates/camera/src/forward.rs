...
/// The DLL this library stands in for, as installed next to the game.
pub const PROXY_NAME: &str = "winhttp.dll";
...
fn system_dll() -> usize {
    ...
    path.extend(PROXY_NAME.encode_utf16());
    path.push(0);
    let h = unsafe { win::LoadLibraryW(path.as_ptr()) } as usize;
    HANDLE.store(h, Ordering::Release);
    h
}

fn resolve(cache: &AtomicUsize, name: &'static str) -> usize {
    let f = cache.load(Ordering::Acquire);
    if f != 0 {
        return f;
    }
    let module = system_dll();
    if module == 0 {
        return 0;
    }
...
macro_rules! forward {
    ($($name:ident)*) => {$(
        ...
        pub unsafe extern "system" fn $name(
            ...
            let target = resolve(&CACHE, concat!(stringify!($name), "\0"));
            if target == 0 {
                return 0;
            }
