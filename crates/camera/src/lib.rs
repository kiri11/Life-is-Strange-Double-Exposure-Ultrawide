...
//! to the system copy, and on load it finds the three
//! camera patch sites in the mapped executable by signature and writes the
//! bytes RESEARCH.md describes. The file on disk is never touched.
//!
//! Everything that decides *what* to write is plain code over byte slices
//! ([`scan`], [`pe`], [`games`]) and runs on any host, which is what the tests
//! exercise. Only `runtime` and `forward` talk to Windows.
...
/// The release this loader was built for, stamped by the release workflow.
